...
blockades and sending opponents back on a capture. Several twentieth-century board
games are commercial adaptations of Pachisi; Parjamie is not one of them and is not
derived from any of them.

## Rules for this project

- **Never use the trademarked names of those commercial adaptations**, or any close
  spelling of one. Those names belong to the companies that own them, and Parjamie has
  no connection to them. This applies to on-screen text, the home screen name, the icon,
  code comments, test names, commit messages, and documentation. Name the real ancestor
  instead — Pachisi — or describe the game as "the race home" or a "cross-and-circle
  game". `Tests/ParjamieEngineTests/NamingTests.swift` fails if a forbidden spelling
